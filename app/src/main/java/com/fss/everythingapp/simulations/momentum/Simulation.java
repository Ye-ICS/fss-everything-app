...
public void update(double dt) {
    ...
    for (int i = 0; i < pucks.size(); i++) {
        ...
        if (pos.x - radius <= 0) {
            pos = new Vector2D(radius, pos.y); // snap inside left wall
            vel = new Vector2D(Math.abs(vel.x) * elasticity, vel.y); // reflect x
            puck.setPosition(pos);
            puck.setVelocity(vel);
        } else if (pos.x + radius >= width) {
            pos = new Vector2D(width - radius, pos.y); // snap inside right wall
            vel = new Vector2D(-Math.abs(vel.x) * elasticity, vel.y); // reflect x
            puck.setPosition(pos);
            puck.setVelocity(vel);
        }
        ...
        if (pos.y - radius <= 0) {
            pos = new Vector2D(pos.x, radius); // snap inside top wall
            vel = new Vector2D(vel.x, Math.abs(vel.y) * elasticity); // reflect y
            puck.setPosition(pos);
            puck.setVelocity(vel);
        } else if (pos.y + radius >= height) {
            pos = new Vector2D(pos.x, height - radius); // snap inside bottom wall
            vel = new Vector2D(vel.x, -Math.abs(vel.y) * elasticity); // reflect y
            puck.setPosition(pos);
            puck.setVelocity(vel);
        }
    ...
    // Debugging output to show puck positions
    for (Puck puck : pucks) {
        Vector2D pos = puck.getPosition();
